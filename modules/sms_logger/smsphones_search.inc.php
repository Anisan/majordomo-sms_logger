...
*/
 global $session;
  ...
  if (!$sortby_gpslocations) {
   $sortby_gpslocations=$session->data['smsphones_sort'];
  } else {
   if ($session->data['smsphones_sort']==$sortby_gpslocations) {
   ...
   $session->data['smsphones_sort']=$sortby_gpslocations;
  }
  if (!$sortby_gpslocations) $sortby_gpslocations="TITLE";
  $out['SORTBY']=$sortby_gpslocations;
  // SEARCH RESULTS
  $res=SQLSelect("SELECT * FROM sms_phones WHERE $qry ORDER BY ".$sortby_gpslocations);
  if ($res[0]['ID']) {
   colorizeArray($res);
   $total=count($res);
   for($i=0;$i<$total;$i++) {
